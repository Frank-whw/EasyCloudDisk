...
EasyCloudDisk 是一个基于 Spring Boot 后端框架与阿里云 OSS 云存储服务构建云存储系统。系统实现了完整的多用户文件管理生态，包括用户认证授权、文件基础操作（上传、下载、删除）、高级文件传输功能（秒传、断点续传、差分同步）以及实时文件变更通知等能力。采用前后端分离的架构：后端负责业务逻辑处理、用户认证、安全管控以及与阿里云 OSS 的交互；前端提供直观流畅的用户界面，通过封装的 API 客户端与后端进行通信。

### 二、目录结构与模块

项目采用多模块架构，包含后端服务、前端界面、Java客户端、Python同步客户端以及部署脚本。以下是主要目录结构：

- `server/` 后端服务（Spring Boot）
  - `src/main/java/com/clouddisk/` 核心代码
    ...
    - `storage/` OSS 存储实现（上传/下载/删除/存在性/建桶/健康）
    - `config/` Spring/阿里云/OpenAPI/CORS 等配置
  - `src/main/resources/application.yml` 端口、数据源、JPA、JWT、CORS、OSS 配置
  - `pom.xml` Maven 配置
  - `logs/` 运行日志
  - `temp_chunks/` 临时分块存储
  - API 文档：`API_AUTH_DOCUMENTATION.md`
- `frontend/` 前端静态页面与脚本
  - `index.html`、`share.html` 主页面
  - `css/main.css` 样式文件
  - `js/` 模块化脚本
  ...
  - 文档：`分享功能快速指南.html`、`debug-auth.html`、`debug-share.html`
- `client/` Java 客户端 SDK 与同步组件（Maven 项目）
  - `src/main/java/com/clouddisk/client/` 客户端代码
    - `http/` HTTP 客户端（FileApiClient）
    - `sync/` 同步组件（DirectoryWatcher、SyncManager、ConflictResolver）
    - `util/` 工具类（HashCalculator、CompressionService）
  - `src/test/` 单元测试
  - `pom.xml` Maven 配置
  - `upload/`、`local/` 测试目录
  - 文档：`启动说明.md`、`重新编译运行.md`、`TODO.md`
- `sync_client/` Python 同步示例脚本
  - `sync_client.py` 主同步脚本（使用 watchdog 监听目录，sseclient 订阅 SSE）
  - `requirements.txt` Python 依赖
  - `sync_folder/` 同步文件夹示例
- `scripts/` 部署相关脚本
  - `auto-deploy.sh` 自动部署脚本
  - `config.sh` 配置脚本
  - `server-startup.sh` 服务器启动脚本
  - `setup-ssh.sh` SSH 设置脚本
- `test/` 接口联调与测试脚本
  - 认证测试：`test_auth_login.sh`、`test_auth_register.sh`
  - 文件操作测试：`test_file_upload.sh`、`test_file_download.sh`、`test_file_delete.sh`、`test_file_list.sh`
  - 高级功能测试：`test_chunk_deduplication.sh`、`test_resumable_upload.sh`、`test_delta_sync.sh`、`test_encryption.sh`、`test_collaboration_share.sh`
  - 健康检查：`test_health.sh`
  - 配置：`config.example.sh`、`config.sh`
  - 文档：`README.md`
- `EasyCloudDisk-hy/` 另一个项目分支/版本
  - 包含完整的 `client/`、`frontend/`、`scripts/`、`server/`、`sync_client/` 子目录
  - 前端静态页面与脚本（实际前端代码位于此目录）
    - `index.html`、`share.html` 主页面
    - `css/main.css` 样式文件
    - `js/` 模块化脚本
      - `config.js` 全局配置（API 地址、上传阈值、存储键名）
...
接口清单
  ...
  - 共享：`/shares`（创建）、`/shares/owned`、`/shares/access`、`/shares/{id}`（撤销）、`/shares/{id}/contents`
  - 版本：`/files/{id}/versions`、`/files/{id}/versions/{version}/restore`
  - 健康：`/health`
...
- 找到一个可用的云
  ...
  **云平台部署**
  ...
  - 云平台与组件：选择阿里云部署，后端运行在 ECS（Ubuntu LTS），对象存储使用 OSS。
  - 部署目标：后端业务逻辑运行于云虚拟机（ECS），用户数据存储于云对象存储（OSS）。

- 使用云对象存储来存储用户数据

  **OSS服务实现:**

  - 核心接口 ： OSSService.java 定义了基本的文件操作方法（上传、下载、流式下载、删除）
  - 具体实现 ： OSSServiceImpl.java 提供了与阿里云 OSS 交互的完整逻辑
  - 主要功能 ：
  ...
  - 配置类 ： OSSConfig.java 负责创建和配置OSSClient实例 

- 业务逻辑部署在云虚拟机

  **部署环境**

  - 服务器环境 ：在ECS虚拟机上运行Spring Boot应用
...
- 创建 OSS 存储桶
  ...
  - 在 OSS 创建 `clouddisk-<your-bucket>`；选择区域与访问策略

- 准备 RAM 用户与权限

  - 为 ECS 创建 RAM 用户，授予 OSS 的最小权限（list/get/put/delete 对指定桶）
  - 将该用户密钥绑定到后端 ECS 实例，避免使用明文 Access Key
...
- 服务配置与启动
  ...
  - 设置环境变量：`OSS_ENDPOINT`、`OSS_BUCKET_NAME`（若未使用 RAM 用户，则另设 `OSS_ACCESS_KEY_ID`、`OSS_ACCESS_KEY_SECRET`）。
  - 在 `server/` 运行：`mvn spring-boot:run -DskipTests`；或打包为 JAR 并用 `scripts/server-startup.sh` 后台运行。

- 验证与联通

  - 打开健康检查 `http://<ECS-IP>:8080/health`；配置前端 `API_BASE_URL` 指向该地址。
...
- 实现一个客户端，并实现本地与云端的自动同步
  ...
  - `SyncManager` 以 `DirectoryWatcher` 为本地入口，结合 `HashCalculator` + `CompressionService` 在检测到 CREATE/MODIFY 时先算哈希、再压缩并通过 `FileApiClient` 执行秒传检测、OSS 直传或 API 上传；线程池异步执行，预留 `synchronizeRemoteChanges()` 与冲突解决逻辑。  
  ...
  fileApiClient.checkFileExists(contentHash)... ossService.uploadFile(...) ... fileApiClient.uploadFile(request);
  ```
...
- 数据压缩
  ...
  - **在上传文件前，客户端应对文件进行压缩**:
  - **上传端策略**：服务端在 `StorageService.storeFile`/`storeBytes` 中根据 `compress` 标志对原始字节进行 GZIP 压缩，并在 OSS 元数据里记录 `original-size`、`compressed`、`sha256` 等字段，保证上传负载只包含压缩后的 payload。  

  ```46:84:server/src/main/java/com/clouddisk/storage/OssStorageService.java
  ...
  ```132:147:server/src/main/java/com/clouddisk/storage/OssStorageService.java
  public InputStream loadFile(String storageKey, boolean decompress) {
      ResponseInputStream<GetObjectResponse> response = ossClient.getObject(request);
      if (decompress) {
...
* 块级去重
  ...
  * **固定块切分**：`ChunkService` 将文件按 4 MB 分块并计算每块 SHA-256；同时根据 `compress` 决策对块进行 GZIP 压缩。  
  ...
  - **仅上传新块**：`uploadNewChunk` 在 GZIP 后如果发现压缩反而变大，会回退到原始数据，保证传输最优；并把 `chunkHash` 映射到 OSS 中的唯一对象，引用计数支撑多文件共享。  
...
```18:158:EasyCloudDisk-hy/frontend/js/auth.js
...
```3:225:EasyCloudDisk-hy/frontend/js/api.js
...
```10:399:EasyCloudDisk-hy/frontend/js/fileManager.js
...
```41:153:EasyCloudDisk-hy/frontend/js/uploadManager.js
...
```3:76:EasyCloudDisk-hy/frontend/js/hashCalculator.js
...
```3:166:EasyCloudDisk-hy/frontend/js/syncManager.js
...
- **使用指引与结构**  
  - `EasyCloudDisk-hy/frontend/README.md` 已提供启动步骤（本地静态服务 `python -m http.server 3000`）、功能覆盖、测试场景（秒传、跨端同步）以及目录结构说明，方便快速定位 `js/css` 模块。
...
- 后端：REST API + 业务服务 + 存储抽象
  ...
  - 存储层：`storage/StorageService` 接口与 `OssStorageService` 实现，负责对象存储与元数据
...
export OSS_BUCKET_NAME="clouddisk-test-1762861672"
```
- 启动并测试编译
```bash
mvn spring-boot:run 
...
- 如使用 OSS，观察后端日志与对象存储中对象的变化（建桶/上传/删除）
