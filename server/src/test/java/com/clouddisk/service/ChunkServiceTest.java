...
class ChunkServiceTest {
    ...
    void testAssembleFile_Success() throws Exception {
        ...
        when(storageService.loadFile("s3-key-1", false))
                .thenReturn(new ByteArrayInputStream(new byte[1024]));
        when(storageService.loadFile("s3-key-2", false))
        ...
        verify(storageService, times(1)).loadFile("s3-key-1", false);
        verify(storageService, times(1)).loadFile("s3-key-2", false);
    }
    ...
    void testDeleteFileChunks_RefCountZero_DeletesChunk() {
        ...
        verify(storageService, times(1)).deleteFile("s3-key");
