...
# 执行上传请求
echo "发送请求: POST $API_BASE_URL/api/files/upload"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    if echo "$HTTP_BODY" | grep -qE '"success":\s*true'; then
        ...
        OSS_KEY=$(echo "$HTTP_BODY" | grep -oE '"oss_key":\s*"[^"]*' | cut -d'"' -f4 || echo "")
        ...
        if [ -n "$FILE_ID" ]; then
            ...
            echo "文件名称: $TEST_FILE_NAME"
            [ -n "$OSS_KEY" ] && echo "OSS Key: $OSS_KEY"
            ...
            if echo "$LIST_RESPONSE" | grep -q "$FILE_ID"; then
                ...
                DOWNLOAD_CODE=$(curl -s -o /dev/null -w "%{http_code}" \
                    -X GET "$API_BASE_URL/api/files/$FILE_ID/download" \
