...
echo "发送请求: POST $API_BASE_URL/api/files/resumable/init"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/resumable/init" \
...
" 2>/dev/null || {
    ...
    RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
        -X POST "$API_BASE_URL/api/files/resumable/$SESSION_ID/chunk/$CHUNK_INDEX" \
        -H "Authorization: Bearer $AUTH_TOKEN" \
...
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/resumable/$SESSION_ID/complete" \
