#!/bin/bash
# 断点续传会话列表测试脚本
# 测试 GET /files/resumable/sessions 接口
...
# 执行获取会话列表请求
echo "发送请求: GET $API_BASE_URL/api/files/resumable/sessions"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X GET "$API_BASE_URL/api/files/resumable/sessions" \
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    if echo "$HTTP_BODY" | grep -qE '"success":\s*true'; then
        # 尝试提取会话数量
        SESSION_COUNT=$(echo "$HTTP_BODY" | grep -oE '"sessionId":' | wc -l || echo "0")
        echo -e "${GREEN}✓ 测试通过: $TEST_NAME${NC}"
        echo "会话列表获取成功"
        echo "会话数量: $SESSION_COUNT"
        if [ "$SESSION_COUNT" -gt "0" ]; then
            echo "已找到 $SESSION_COUNT 个上传会话"
        fi
        exit 0
    else
