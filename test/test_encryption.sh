...
echo "步骤 1: 上传加密文件"
echo "发送请求: POST $API_BASE_URL/api/files/upload-encrypted"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/upload-encrypted" \
...
echo "发送请求: GET $API_BASE_URL/api/files/$FILE_ID/encryption"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X GET "$API_BASE_URL/api/files/$FILE_ID/encryption" \
...
echo "发送请求: POST $API_BASE_URL/api/files/convergent-check"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/convergent-check" \
    -H "Authorization: Bearer $AUTH_TOKEN" \
