...
UPLOAD_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${API_BASE_URL}/api/files/upload" \
  ...
  -F "file=@${TEST_FILE};filename=test_versions.txt" \
  -F "path=/")
...
echo ""
echo "[步骤 4] 获取文件版本历史..."
VERSIONS_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "${API_BASE_URL}/api/files/${FILE_ID}/versions" \
  -H "Authorization: Bearer ${AUTH_TOKEN}")
...
if [ "$HTTP_CODE" -eq 200 ]; then
    echo "✓ 版本历史获取成功"
    VERSION_COUNT=$(echo "$RESPONSE_BODY" | grep -o '"versionNumber":' | wc -l)
