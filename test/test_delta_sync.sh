...
echo "发送请求: GET $API_BASE_URL/api/files/$FILE_ID/signatures"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X GET "$API_BASE_URL/api/files/$FILE_ID/signatures" \
...
echo "发送请求: POST $API_BASE_URL/api/files/$FILE_ID/delta"
echo "注意: 差分同步可能需要特定的数据格式，此测试可能因实现而异"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/$FILE_ID/delta" \
    -H "Authorization: Bearer $AUTH_TOKEN" \
