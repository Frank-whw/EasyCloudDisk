...
echo "发送请求: POST $API_BASE_URL/api/files/quick-check"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/quick-check" \
    -H "Authorization: Bearer $AUTH_TOKEN" \
...
echo "发送请求: POST $API_BASE_URL/api/files/quick-upload"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
    -X POST "$API_BASE_URL/api/files/quick-upload" \
    -H "Authorization: Bearer $AUTH_TOKEN" \
    -H "Content-Type: application/json" \
